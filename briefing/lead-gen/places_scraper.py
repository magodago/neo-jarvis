...
# -*- coding: utf-8 -*-
"""
NEO MASSIVE LEAD GENERATOR — Google Places API v1 + email scraping
Genera cientos de leads con teléfono, web y email.
"""
...
def extract_email(url, timeout=8):
    ...
    try:
        ...
        for e in found:
            ...
            # Preferir emails con dominio propio (no gmail/yahoo/hotmail)
            if e.split('@')[1] not in ('gmail.com','yahoo.es','yahoo.com','hotmail.com','outlook.com'):
                return e
        # Si no hay email de dominio propio, coger el primero válido
        for e in found:
            e = e.strip().lower()
            if any(s in e for s in SKIP_EMAILS): continue
            if len(e) > 5 and '@' in e:
...
def search_places(query, sector, city, page_token=None):
    """Llama a Places API v1 y devuelve resultados"""
...
def scrape_all():
    ...
    for idx, (sector, city) in enumerate(SECTORS_CITIES):
        ...
        page_token = None
        page = 0
        while page < 3:  # max 3 páginas por query = hasta 60 resultados
            ...
            for p in places:
                ...
                if existing:
                    # Si ya existe pero no tiene email, intentar extraerlo
    ...
    conn.close()
    print(f"\n{'='*50}")
    print(f"📊 TOTAL: {total_nuevos} leads nuevos, {total_emails} con email")
    print(f"{'='*50}")
